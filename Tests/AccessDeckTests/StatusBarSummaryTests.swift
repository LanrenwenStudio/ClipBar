...
struct StatusBarSummaryTests {
    ...
    @Test("Status bar and widgets share five-hour reset buckets")
    func sharesFiveHourResetBuckets() {
        ...
        for (raw, expected) in cases {
            #expect(StatusBarSummary.shortResetText(from: raw) == expected)
        }
        ...
        let provider = ProviderWidgetData(
            ...
            healthyCount: 2,
            statusText: "正常",
            windows: [QuotaWindowSummary(label: "5h 限制", remainingPercent: 75, resetText: "3h 12m")]
        )
        #expect(provider.nearestFiveHourDisplayText == "3h")
    }
