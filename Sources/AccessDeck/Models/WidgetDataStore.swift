...
final class WidgetDataStore: @unchecked Sendable {
    ...
    static let appGroupID = "group.com.lanrenwen.accessdeck"
    ...
    func saveSnapshot(_ snapshot: AccessDeckWidgetSnapshot) {
    ...
    func loadSnapshot() -> AccessDeckWidgetSnapshot {
        if let shared = sharedDefaults, let data = shared.data(forKey: snapshotKey),
           let snapshot = try? JSONDecoder().decode(AccessDeckWidgetSnapshot.self, from: data) {
            return snapshot
        }
    ...
    public func syncFrom(accounts: [AccountQuota], settings: AppSettings, connection: ConnectionState) {
        ...
        let snapshot = AccessDeckWidgetSnapshot(
            lastUpdated: Date(),
