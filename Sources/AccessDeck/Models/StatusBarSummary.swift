...
enum StatusBarSummary {
    ...
    private static func nearestResetText(
        ...
        return shortResetText(from: raw)
    }

    /// Formats a quota reset countdown the same way as the macOS status bar:
    /// show the whole remaining hour, with sub-hour values represented as 1h.
    static func shortResetText(from raw: String?) -> String? {
        guard let raw else { return nil }
        let normalized = raw.trimmingCharacters(in: .whitespacesAndNewlines)
        guard let seconds = resetDurationInSeconds(normalized) else { return nil }
